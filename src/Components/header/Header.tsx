...
export function Header() {
  ...
  var resultSearch = items.filter(items=> items.title.match(new RegExp(searchValue, "i")));
  ...
  return (
    ...
    <header>
      ...
      <div className='Search'  > 
        ...
        value={searchValue}
        onChange={(event)=>{handlSearch(event);}}
      />
      </div>

      {searchValue && (<div className='box' id='box' onBlur={()=>{setSearchValue('')}}>
        {  
           !resultSearch.length 
           ? "No result"
           : resultSearch.map((items:IProd)=>
            {
              return(
                <Link onClick={()=>{setSearchValue('')}}  to={`/products/${items.id}`} key={items.id}>
                <div className='SearchItem'>
                <img className='SearchImg'   src={items.image} alt=""/>
                <div className='SearchTitle'><p>{items.title.slice(0,36)}</p></div>
                </div>
                </Link>
              )
            }
